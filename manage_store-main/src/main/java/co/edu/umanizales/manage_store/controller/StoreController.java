...
public class StoreController {
    ...
    public ResponseEntity<ResponseDTO> createStore(@RequestBody Store store) {
        ...
        if (findStore == null) {
            ...
            return new ResponseEntity<>(new ResponseDTO(409, "Ya existe una tienda con ese código", null), HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public ResponseEntity<ResponseDTO> getStoreById(@PathVariable String code) {
        ...
        if (findStore == null) {
            return new ResponseEntity<>(new ResponseDTO(404, "No existe una tienda con ese código", null), HttpStatus.OK);
        } else {
